...
define(["jquery"], function($) {
	...
	// 默认配置。
		DEFAULT_CONFIG = {
			...
			bottomPath: null,//B2C1.4.0新增，在app内调用图片预览，底部浮层地址（按钮+文字）。本地调试未拓展
			jsOnResume: null
		},
	...
	$.extend(Fn.prototype, {
		...
		_setCenter: function(index) {
			var _self = this,
				jImgs = _self._getAllPicItems().eq(index),
                jThis = jImgs,
		...
		_loadImg: function(index) {
			...
			var jImgs = _self._getAllPicItems().eq(index),
                jThis = jImgs,
                sSrc = _self._getImgSrc(jThis),
				jParentBox = jThis.parents("li");
						...
						if (_self.config.canPreview) {
							require(["photoSwipe"], function(PhotoSwipe) {
								...
								new PhotoSwipe({
									...
									bottomPath: _self.config.bottomPath,
									jsOnResume: _self.config.jsOnResume,
