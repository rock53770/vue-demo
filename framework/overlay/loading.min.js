...
define(["jquery", "util", "./base.min", "say"], fnOverlayLoading);
...
function fnOverlayLoading($, Util, Overlay, Say) {
	...
	var _loading = {
		...
		show: function(oConfig){
			...
			_self.loadingInstance && _self.loadingInstance.hide();//先把上一个Loading干掉
...
// console.log("showLoading");
		...
		hide: function(){
			...
			//调用webview接口
			if(GConfig.isInApp){
				if(window.isShowPageLoading === true){//关闭native的页面级loading
					window.isShowPageLoading = false;
					wv.loading().hide();
				} else{//关闭普通Loading
					wv.hideSubmiting();
				}
