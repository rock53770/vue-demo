 ...
 * @author xiaochai.dong@guanxin.com
 */
define(["jquery", "util", './photoSwipe.less'], function($, Util) {
	var LAYER_CONTAINER_HTML = '<div class="photoSwipe-layer">' + '<div class="item-box"><ul class="ks-clear"></ul></div>' + '<div class="icon-box"></div>' + '</div>',
		...
		DEFAULT_CONFIG = {
			...
			// jsOnResume: null,//模仿openpage
			errSrc: "./img/banner-defaultD.png"
		},
		...
		Fn = function(oConfig) {
			...
			if (GConfig.isInApp) {
				...
				wv.showPhoto(asSrc, oConfig.currentIndex || 0, true, oConfig.bottomPath); //默认轮播图的这个可以支持app端保存到图片
				return;
			}
		...
		_fnFlyOpen: function() {
			...
			function callback(){
				...
				setTimeout(function() {
					...
					_self.jLayerContainer.css({
                        opacity: 1,
                        height: (navigator.userAgent.indexOf("Safari") > -1 ? window.innerHeight : $(window).height()) + 'px'
                    });
