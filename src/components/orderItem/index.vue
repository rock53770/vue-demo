<template>
  <div class="pt10 bg">
    <div @click="opationsRout" class="bg-white">
        <div :class="['item-user-name','flex-center','justify-sb','h45','half-border-bottom','plr12','ft14', item.showDetail ? 'arrow-block-right': '']">
        ...
        <div class="half-border-bottom pt10 pb10 plr12 ft14">
            <div v-for="(i, index) in item.quoteList" :key="index" class="lh45">
                <div v-if="type == 1">
                    <img :src="i.icon" width="36" height="36" class="ver-align-mid mr5 bdr-50" style="border: 1px solid #eeeeee;">
                    <span>{{ i.companyName }}：</span>
                    <span v-if="i.price">{{ i.price |currency2('￥','2') }}</span>
                    <em v-if="i.handlerName" class="cl-orange fs-n">&nbsp;{{ i.handlerName }}</em>
                </div>
    ...
    <div :class="['h45','plr12', 'flex-center', 'justify-sb','van-hairline--bottom','bg-white',type == 3 ? 'ta-l' : 'ta-r']">
        <span class="cl-gray f12">{{ $util.formatDate(item.handlerTime, 'yyyy-MM-dd HH:mm') }}</span>
        <div>
            <button class="h28 w65 bg-white cl-black bdc bdr3" @click="deleteItem" v-if="item.delete">关闭订单</button>
            <button class="h28 w65 cl-white bdr3 ft12 van-button--primary" v-if="item.pay" @click="goPay">立即支付</button>
            <button class="h28 plr5 cl-white bdr3 van-button--primary" v-if="item.address" @click="selectAddress">选择取件方式</button>
        </div>
    </div>
  </div>
</template>
<script>
/**
 * 说明，props内type对应三种不同的状态，分别为全部，交易中，已完成
 * @item, 订单对象
 * @index, 当前列表渲染下标值
 */
export default {
