<template>
    <div class="balanceAll">
        <btScroller :getData="getData" :currentPage.sync="pageIndex" :propType="1" ref="bAll">
            <div>
                <!-- 列表主体 -->
...
export default {
    ...
    methods:{
        symbol(item){//加减符号
            let str = '';
            if(item.flowVoTypeValue==6){
                str = '-'
            }else if(item.flowVoTypeValue==3|| item.flowVoTypeValue==5){
                str = ''
            }else{
                str = '+'
            }
