<template>
    <div class="address-box">
        <div>
            <ul class="pt10">
                <li class="plr10 mb10 bg-white" v-for="(i, index) in list" :key="index">
                    ...
                    <div class="flex-center h35 ft14 cl-gray">
                      <span class="flex-1" :class="[i.isDefault?'icon-elect cl-orange':'icon-select']"
                       v-text="i.isDefault?'默认地址':'设为默认地址'" @click="setDefault(i)"></span>
                      <div class="">
            ...
            <div class="h70">
              <div class="flex-center pos-f b0 bg-white h70 p10 w-100 zi10">
                <van-button round type="primary flex-1" @click="onAdd" text="添加新地址"></van-button>
              </div>
            </div>
        </div>
        <van-popup class="form-container h-100" v-model="isshowedit" position="top" :overlay="true">
            <van-cell-group>
                <van-field v-model="newaddress.reciverName" clearable label="联系人" placeholder="请输入联系人" :error='newaddress_error.reciverName' />
                <van-field v-model="newaddress.reciverPhone" type="tel" clearable label="联系号码" placeholder="请输入联系电话" :error='newaddress_error.reciverPhone' />
                <van-field v-model="reciverProvinceCityCount" disabled label="邮寄地址" placeholder="请选择地址" :error='newaddress_error.reciverCountyCode'>
                    <div @click="selectsite" slot="button" class="float-module"></div>
                </van-field>
                <van-field v-model="newaddress.reciverAddress" clearable label="详细地址" placeholder="请输入详细地址" :error='newaddress_error.reciverAddress' />
            </van-cell-group>
            <div class="flex-center mlr15 mt24">
              <van-button round type="primary flex-1 mlr15" @click="isshowsite=isshowedit=false" text="取消"></van-button>
        ...
        <van-popup v-model="isshowsite" position="bottom" :overlay="false">
            <van-area :area-list="areaList" title="选择地址" @change="onChange" @cancel="onCancel" @confirm="onConfirm" />
        </van-popup>
    </div>
</template>
...
//                    Toast(res.message)
        ...
        onSelect(i){
            if(GConfig.isInApp){
              ...
              Util.setSessionData("selectAddress",i)
              btPage.goBack();
            }
        },
        onAdd() {
        ...
        onEdit(i) {
        //   this.$router.push({ name: 'address-edit', query:{
        //       id:i.ID
        //   }});
