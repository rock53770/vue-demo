    ...
    <div class="orderDetail pos-r ft16 of-hide" :style="{'padding-bottom' : showToolsBtn ? '70px':0}">
        <template v-if="!isLoading">
            <div class="address bg-white p12 mb10 ft14">
                <p class="mb8" v-if="dataObject.isShowLogisticsInfo && dataObject.logisticsInfo.name && dataObject.logisticsInfo.phone"><span class="mr10">{{ dataObject.logisticsInfo.name }}</span><span>{{ dataObject.logisticsInfo.phone | phone }}</span></p>
                <p class="mb8 cl-gray" v-if="dataObject.isShowLogisticsInfo && dataObject.logisticsInfo.type !==3 && dataObject.logisticsInfo.type !== 5">{{ dataObject.logisticsInfo.provinceName }}{{ dataObject.logisticsInfo.cityName }}{{ dataObject.logisticsInfo.countyName }}{{ dataObject.logisticsInfo.address }}</p>
            ...
            <div class="bg-white mb10 ft" v-if="dataObject.commercialPolicyNo || dataObject.compulsoryPolicyNo">
                ...
                <div class="van-hairline--bottom pt12 pb12 ft14">
                    <p class="plr12 mb10" v-if="dataObject.commercialPolicyNo">商业险：{{ dataObject.commercialPolicyNo }}</p>
                    <p class="plr12" v-if="dataObject.compulsoryPolicyNo">交强险：{{ dataObject.compulsoryPolicyNo }}</p>
                </div>
            </div>
            <!-- 商业险 -->
                    ...
                    <div class="h60 pt8 plr12 van-hairline--top van-hairline--bottom">
                        <div class="ta-r ft14 mb8 flex-center">
                            <div style="width: 34%" class="fw-b ta-l">
                                <p>折扣率：<strong class="cl-orange" v-if="dataObject.discountRate">{{ dataObject.discountRate|currency2 }}</strong></p>
                            </div>
                            <div style="width: 22%"></div>
                            <div style="width: 22%" class="fw-b cl-orange">合计：</div>
                            <div style="width: 22%" class="ta-r fw-b cl-orange">{{ dataObject.commercialInsurancePremium|currency2 }}</div>
                        </div>
            ...
            <div class="bg-white mb10">
                ...
                <div class="ft14 plr12 pb10 van-hairline--bottom">
                    ...
                    <!-- <div class="flex-center mb8">
                        <img src="../../assets/icons/gift.png" width="20" height="18">
                        <span class="ml10 w50">奖励</span>
                        <span class="cl-orange">{{ dataObject.reward|currency('￥','2') }}</span>
                    </div> -->
            ...
            <!-- 订单完成状态 -->
            <div :class="['type', dataObject.handlerStatus == -2 ? 'close': 'success']" :style="{ 'top': dataObject.invoice ? '157px':'60px' }">{{ dataObject.handlerStatus == -2 ? '订单关闭': '投保成功' }}</div>

            <!-- buttons -->
            <div class="bg-white plr12 ft16 h70 pos-f b0 l0 r0 van-hairline--top flex-center" v-show="showToolsBtn">
                <van-button round type="default" @click="detailShare" class="mr10 bd-orange flex-1 cl-orange p0">分享</van-button>
                <a class="flex-1 cl-orange bd-orange ta-c h50 bdr25 lh50" href="tel:4006038787">联系客服</a>
            </div>
