    ...
    <van-tabs v-model="active">
      <van-tab title="寄送">
        <div class="address-box pt10">
          <div>
            <van-cell-group>
              <van-field v-model="newaddress.reciverName" clearable label="收件人" placeholder="请输入姓名"/>
              <van-field v-model="newaddress.reciverPhone" type="tel" clearable label="手机号" placeholder="请输入手机号" />
              ...
              <van-field v-model="newaddress.reciverAddress" type="textarea" autosize rows="1" clearable label="详细地址" placeholder="请输入详细地址"/>
            </van-cell-group>
            <div class="bg-white lh50 plr10 mt10 ta-c" @click="addressListSelect">
                <img class="w20 ver-align-mid" src="../../assets/icons/add@2x.png"/>
                <span class="cl-orange ver-align-mid">选择现有配送地址</span>
            </div>
          </div>
          <van-popup v-model="isshowsite" position="bottom" :overlay="false">
            <van-area :area-list="areaList" :value="newaddress.reciverCountyCode ? newaddress.reciverCountyCode.toString() : ''" @cancel="onCancel" @confirm="onConfirm" />
          </van-popup>
        </div>
      </van-tab>
      <van-tab title="自取">
...
import { Toast } from 'vant';
    export default {
        ...
        data() {
            return {
                ...
                list2: [{
                  ...
                  id:2,
                  desc:"浙江省杭州市浙江省杭州市浙江省杭州市浙江省杭州市浙江省杭州市浙江省杭州市浙江省杭州市"
                }],
                selfAddress: []
            };
        },
        ...
        methods: {
                ...
                btPage.open({
                    ...
                    jsOnResume: (res) => {
                      if(GConfig.isInApp){
                        ...
                        this.reciverProvinceCityCount = this.newaddress.reciverProvinceName + ' ' + this.newaddress.reciverCityName + ' ' + this.newaddress.reciverCountyName
                      }
                    }
                })
            },
            addressQuery(){
                ...
                myService.addressQuery({ID:this.id}).then((res) => {
                    if(res.code == 1){
                      ...
                      if(Util.getSessionData("selectAddress")){
                        address = Util.getSessionData("selectAddress");
                        this.newaddress = address
                        this.reciverProvinceCityCount = address.reciverProvinceName + ' ' + address.reciverCityName + ' ' + address.reciverCountyName
                      } else {
                        if(res.object){
                          this.newaddress = address
                          this.reciverProvinceCityCount = address.reciverProvinceName + ' ' + address.reciverCityName + ' ' + address.reciverCountyName
                ...
                myService.addressList({}).then((res) => {
                    if(res.code == 1){
                        res.object.forEach(it => {
                            if (it.isDefault) {
                                self.newaddress = it
                                self.reciverProvinceCityCount = it.reciverProvinceName + ' ' + it.reciverCityName + ' ' + it.reciverCountyName
                            }
                        })
                    }
                })
            ...
            getSelfAddress() {
                let param = {}
                orderService.getSelfAddress(param)
                    .then((res) => {
                        if (res.code == 1) {
        ...
        beforeRouteEnter(to, from, next) {
            ...
            if (from.name !== 'addressListSelect') {
                Util.removeSessionData("selectAddress");
            }
            next()
        },
        mounted() {
