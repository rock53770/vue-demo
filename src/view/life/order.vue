...
<template>
    <div class="order-box w-100 pos-a l0 r0 b0" :style="{ top: navHeight + 'px' }">
        <div class="search-name-box pos-r van-hairline--bottom">
            <van-search v-model="searchObject.key" background="#ffffff" placeholder="投保人或被保人姓名"  @search="onSearch" show-action>
        ...
        <div class="w-100 pos-r bg-white dryTab">
                <div class="dry-tab">
                    <div class="tabs">
                        <div class="tabs-head flex-center justify-sb van-hairline--bottom pos-r h-100">
                            <div class="tabs-left flex-center justify-sb h-100 pos-r">
                        ...
                        <div v-for="(item, index) in tabs" :key="index">
                            <div v-if="index == tabsIndex" class="tabs-pane">
                                <btScroller :ref="'scroll_box_'+index" :getData="getData" :currentPage.sync="item.way.currentPage" :propType="tabsIndex">
                                    <lifeItem v-for="(item, index) in item.way.items" :key="index"
                                        :index="index" :item="item" :type="tabsIndex" v-on:del="promptDel" :getfn="getData">
                                    </lifeItem>
        ...
        <div class="tagIcon bdr-50 ta-c pos-f" @click="goCarOrder" style="right:10px; bottom: 100px;">
            <img src="../../assets/icons/icon_car@3x.png" class="w-100 h-100"/>
        </div>
...
export default {
    ...
    methods: {
        ...
        getData () {
            ...
            $promise = lifeService.lifeOrder(param)
            $promise.then((res) => {
                if (res.code == 1) {
